...
| `public/` | Static files copied verbatim into the build output (currently just `manifest.json`). |
| `extension/` | **Generated** by `vite build` (gitignored). This is the unpacked extension `web-ext` loads/packages. Never edit by hand — edit `public/` or `src/` and rebuild. |
...
pnpm build           # vite build: src/ -> extension/
...
pnpm ext:run         # web-ext run --source-dir extension --target chromium
                      ...
                      #   Run `pnpm build` first — this script does not build.

pnpm ext:build       # pnpm build && web-ext build --source-dir extension \
                      #   --artifacts-dir web-ext-artifacts --overwrite-dest
                      #   -> a shippable web-ext-artifacts/*.zip
```

Manual/e2e check: after `pnpm build && pnpm ext:run`, open
...
- `src/sourcemap.test.ts` — Vitest unit tests, colocated with the module they
  test.
- `public/manifest.json` — the **source** MV3 manifest (edit this).
  `extension/manifest.json` is Vite's copy of it — never edit that one.
- `vite.config.ts` — `root: "src"`, `publicDir` → `public/`, `outDir` →
  `extension/`, two Rollup entry points (`devtools.html`, `panel.html`),
  `assetsInlineLimit: 0` (extension pages need real asset files, not inlined
  base64, under the extension CSP).
